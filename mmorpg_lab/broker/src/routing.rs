    ...
    while let Ok(Some(event)) = network.peer.poll() {
        match event {
            GameNetworkEvent::Connected(conn) => {
                ...
                println!("New connection: Uuid {:?} assigned ID {}", conn.connection_id, new_id);
            }
            ...
            GameNetworkEvent::Message { connection, stream, data } => {
                let Some(msg) = BrokerMessage::from_bytes(&data) else { continue; };
                
                match msg {
                    BrokerMessage::Subscribe { client_id, topic } => {
                        state.topic_subscribers.entry(topic).or_default().insert(client_id);
                        ...
                        if let Some(subscribers) = state.topic_subscribers.get(&topic) {
                            for &client_id in subscribers {
                                if let Some(&client_uuid) = state.id_to_uuid.get(&client_id) {
                                    let out_msg = BrokerMessage::Broadcast { payload: payload.clone() }.to_bytes();
                                    
                                    // Convert Vec<u8> into Bytes for game_sockets
                                    let _ = network.peer.send(&client_uuid.into(), &stream, Bytes::from(out_msg));
                                }
                            }
                        }
                    }
                    
                    BrokerMessage::ClientInput { mut client_id, input } => {
                        ...
                        if client_id == 0 {
                            if let Some(&real_id) = state.uuid_to_id.get(&connection.connection_id) {
                                ...
                                if let Ok(bincode_payload) = bincode::serialize(&welcome) {
                                    let welcome_msg = BrokerMessage::Broadcast { payload: bincode_payload }.to_bytes();
                                    let _ = network.peer.send(&connection, &stream, Bytes::from(welcome_msg));
                                }

                                let default_topic = string_to_topic("shard:0");
                                state.client_to_topic.insert(real_id, default_topic);
                                state.topic_subscribers.entry(default_topic).or_default().insert(real_id);
                                
                                println!("Intercepted Join. Assigned ID: {}. Spawned in shard:0", real_id);
                            }
                        }
                        ...
                        if let Some(topic) = state.client_to_topic.get(&client_id) {
                            if let Some(&shard_uuid) = state.topic_to_shard.get(topic) {
                                ...
                                let _ = network.peer.send(&shard_uuid.into(), &stream, Bytes::from(forward_msg));
                            }
                        }
                    }
